...
.PHONY: all engine tests run-tests clean help sanitize sanitize-engine
...
help:
	...
	@echo "  make sanitize       # build tests with ASan+UBSan"
	@echo "  make sanitize-engine # build engine with ASan+UBSan"
	@echo "  make run-tests      # build and run ./build/perft_runner runfile"
	@echo "  make clean          # remove build/"

# Build everything (tests) with sanitizers
sanitize:
	$(MAKE) CFLAGS='-Iinclude -std=c11 -g -O0 -fsanitize=address,undefined -fno-omit-frame-pointer -Wall -Wextra' all

# Build just the engine with sanitizers
sanitize-engine:
	$(MAKE) CFLAGS='-Iinclude -std=c11 -g -O0 -fsanitize=address,undefined -fno-omit-frame-pointer -Wall -Wextra' engine
