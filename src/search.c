...
static void promote_tt_move_to_front(int *froms, int *tos, int *promos,
                                      int n, int tt_from, int tt_to, int tt_promo)
{
    for (int i = 1; i < n; ++i) {
...
static int search_ab(Position *pos, int depth, int ply, int alpha, int beta)
{
    ...
    for (int i = 0; i < n; ++i) {
        MoveUndo undo;
        make_move(pos, froms[i], tos[i], promos[i], &undo);
        int val = -search_ab(pos, depth - 1, ply + 1, -beta, -alpha);
        unmake_move(pos, &undo);
                             ...
                             int *out_from, int *out_to, int *out_promotion)
{
    ...
    if (n > 0) promote_tt_move_to_front(froms, tos, promos, n, tt_from, tt_to, tt_promo);
    ...
    for (int i = 0; i < n; ++i) {
        MoveUndo undo;
        make_move(pos, froms[i], tos[i], promos[i], &undo);
        int val = -search_ab(pos, depth - 1, 1, -beta, -alpha);
        unmake_move(pos, &undo);
